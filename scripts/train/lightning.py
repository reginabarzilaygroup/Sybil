...
class SybilLightning(pl.LightningModule):
    ...
    def step(self, batch, batch_idx, optimizer_idx, log_key_prefix = ""):
        ...
        for loss_fn in loss_fns:
            local_loss, local_log_dict, local_predictions_dict = loss_fn(model_output, batch, self, self.args)
            loss += local_loss
    ...
    def training_epoch_end(self, outputs):
        ...
        del outputs['loss']
        epoch_metrics = compute_epoch_metrics( outputs, self.args, self.device, key_prefix="train_")
    ...
    def validation_epoch_end(self, outputs):
        if len(outputs) == 0:
            return
        outputs = gather_step_outputs(outputs)
        epoch_metrics = compute_epoch_metrics( outputs, self.args, self.device, key_prefix="val_")
        for k,v in outputs['logs'].items():
            epoch_metrics[k] = v.mean()
        self.log_dict(epoch_metrics, prog_bar = True, logger=True)
    ...
    def test_epoch_end(self, outputs):
        if len(outputs) == 0:
            return
        outputs = gather_step_outputs(outputs)
        epoch_metrics = compute_epoch_metrics( outputs, self.args, self.device, key_prefix=self.save_prefix)
        ...
        metrics_dict = {k[len(self.save_prefix):] :v.mean().item() for k,v in outputs.items() if 'loss' in k}
        metrics_dict.update({k[len(self.save_prefix):] :v.mean().item() for k,v in epoch_metrics.items()})
    ...
    def configure_optimizers(self):
        ...
        if self.args.optimizer == 'adam':
            ...
            optimizer = torch.optim.Adagrad(params, lr=self.args.lr, weight_decay=self.args.weight_decay)
        ...
        scheduler =  {
            'scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, \
                            patience= self.args.patience, factor= self.args.lr_decay,\
