...
INCLUDE_AFTER = datetime.strptime(
    "10-08-2020", "%m-%d-%Y"
)  # annotations made before INCLUDE_AFTER were used to test md.ai
ANNOT_COMMENTS = pd.read_csv("NLST/mdai/annotation_comments_12062020.csv")
...
parser.add_argument(
    ...
    type=str,
    help="JSON exported from md.ai",
    default="NLST/mdai/mdai_mit_project_poBGbqle_annotations_labelgroup_all_2020-11-25-030811.json",
)
parser.add_argument(
    "--output_json_path", type=str, help="Where to export parsed annotations data"
)
parser.add_argument(
    "--output_is_dataset_obj",
    action="store_true",
    default=False,
    help="Whether incorporating annotations into dataset json directly",
)
...
if __name__ == "__main__":
    ...
    for dataset_dict in annotation_metadata_json["datasets"]:
        for annotation_meta_dict in tqdm(dataset_dict["annotations"]):
            ...
            date = datetime.strptime(
                annotation_meta_dict["createdAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            ...
            annotation_meta_dict["data"] = scale_annotations(
                annotation_meta_dict["data"], annotation_meta_dict
            )
            annotation_meta_dict["data"]["user"] = USERS[
                annotation_meta_dict["createdById"]
            ]
    ...
    for series_id in annotation_dict.keys():
        if series_id in list(ANNOT_COMMENTS["series_uid"]):
            if "FF" in str(
                ANNOT_COMMENTS[ANNOT_COMMENTS["series_uid"] == series_id][
                    "comments for Peter"
                ]
            ):
                for img_id in annotation_dict[series_id].keys():
                    annotation_dict[series_id][img_id] = [
                        i
                        for i in annotation_dict[series_id][img_id]
                        if "fintelmann" in i["user"]
                    ]
