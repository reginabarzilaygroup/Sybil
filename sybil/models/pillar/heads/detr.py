...
class DETR3D(AbstractModel):
    ...
    def __init__(self, args, transformer_kwargs, input_dim, num_classes, num_queries):
        ...
        nn.init.kaiming_normal_(
            self.class_embed.weight, mode="fan_out", nonlinearity="relu"
        )
        self.bbox_embed = MLP(
            args,
    ...
    def forward(self, features):
        ...
        features = self.input_proj(
            features
        )  # shape is: [batch_size, hidden_dim, D, H, W]
    ...
    def forward_head(self, features):
        ...
        pos_embed_np = gen_3d_sincos_pos_embed(
            self.transformer_model.d_model, img_seqlen=h, frame_seqlen=d
        )
        pos_embed = torch.tensor(
            pos_embed_np, dtype=features.dtype, device=features.device
        )
        ...
        hs, cross_attn = self.transformer_model.decoder(
            tgt, memory, pos=pos_embed, query_pos=query_embed
        )
