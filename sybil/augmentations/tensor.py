        ...
        self.transform = torchvision.transforms.Compose(
            [
                torchvision.transforms.Normalize(
                    mean=[0.0 for _ in channel_means], std=[1 / s for s in channel_stds]
                ),
                torchvision.transforms.Normalize(
                    mean=[-m for m in channel_means], std=[1.0 for _ in channel_stds]
                ),
            ]
        )
...
class Normalize_Tensor_2d_Img_Per_Chan(Abstract_augmentation):
    ...
    def __init__(self, args, kwargs):
        ...
        assert len(kwargs) == 0
        self.channel_means = (
            [args.img_mean] if len(args.img_mean) == 1 else args.img_mean
        )
    ...
    def __call__(self, img, mask=None, additional=None):
        ...
        return (
            img.sub_(self.channel_means[channel]).div_(self.channel_stds[channel]),
            mask,
        )
