...
class RiskModel(object):
    ...
    def forward(self, batch):
        x_transformed = {key: func(batch[key]) for key, func in self.input_transformers.items()}
...
class PLCOm2012(RiskModel):
    ...
    def input_coef(self):
        coefs = {
                'age': 0.0778868, 
                'race': {'white': 0, 'black': 0.3944778, 'hispanic': -0.7434744, 'asian': -0.466585, 'native_hawaiian_pacific': 0, 'american_indian_alaskan': 1.027152}, 
                'education': -0.0812744, 
                'bmi': -0.0274194, 
                'cancer_hx': 0.4589971, 
    ...
    @property
    def save_keys(self):
        return ['pid', 'age','race','education','bmi','cancer_hx','family_lc_hx','copd','is_smoker','smoking_intensity','smoking_duration','years_since_quit_smoking', 'exam', 'golds', 'probs', 'time_at_event', 'y_seq', 'y_mask', 'screen_timepoint']
