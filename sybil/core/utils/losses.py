...
def compute_annotation_loss(model_output, batch, args):
    ...
    for attn_num in [1,2]:
        ...
        if model_output.get('image_attention_{}'.format(attn_num), None) is not None:
            ...
            # get mask over annotation boxes in order to weigh non-annotated scores with zero when computing loss
        ...
        if model_output.get('volume_attention_{}'.format(attn_num), None) is not None:
            ...
            if isinstance(side_attn, torch.Tensor):
                # attend to cancer side
                cancer_side_mask = (batch['cancer_laterality'][:, :2].sum(-1) == 1).float() # only one side is positive
...
def get_risk_factor_loss(model_output, batch, model, args):
    ...
    for idx, key in enumerate(args.risk_factor_keys):
        ...
        loss = (F.cross_entropy(logit,gold, reduction = 'none') * is_rf_known).sum()/max(1, is_rf_known.sum() )
        total_loss += loss
