    ...
    def create_dataset(self, split_group, img_dir):
        ...
        for mrn_row in tqdm(self.metadata_json):
            ...
            for exam_dict in exams:
                ...
                diff_days = int(exam_dict['diff_days']) # no. of days to the oldest exam (0 or a negative int)
                ...
                for series_id, series_dict in exam_dict['image_series'].items():
                    ...
                    device = -1 if series_data['Manufacturer'] == -1 else DEVICE_ID[series_data['Manufacturer']]
                    slice_thickness = series_data['SliceThickness'] if series_data['SliceThickness'] == '' else float(series_data['SliceThickness']) 
                    ...
                    if (self.args.slice_thickness_filter is not None) and ( (slice_thickness not in self.args.slice_thickness_filter) or (slice_thickness=='') ):
                        continue
                    ...
                    sample = {
                        ...
                        'y_seq': y_seq,
                        'y_mask': y_mask,
                        'exam': int('{}{}'.format(studyuid.replace(".","")[-5:], series_id.replace(".","")[-5:])), # last 5 of study id + last 5 of series id
                        ...
                        'cancer_laterality': np.zeros(3, dtype=np.int), # has to be int, while cancer_location has to be float
